...
export default function ButtonDemo() {
  ...
  const paymentText = "Make payment";
  const startText = "Start";
  
  // Calculate the total duration for the first text to slide out
  const totalPaymentSlideDuration = (paymentText.length - 1) * 30;
        ...
        <div className="relative overflow-hidden w-full h-full z-10">
          <div className="absolute inset-0 flex items-center justify-center w-full">
            <div className="flex text-md font-medium">
              {paymentText.split('').map((char, index) => (
                <span
                  key={`payment-${index}`}
                  className={`
                    ...
                    ${isHovered 
                      ...
                      : `opacity-100 translate-y-0 delay-[${(startText.length + 2) * 30 + index * 30}ms]`
                    }
                  `}
                  style={{
                    transitionDelay: isHovered ? `${index * 30}ms` : `${(startText.length + 2) * 30 + (paymentText.length - index - 1) * 30}ms`
                  }}
                >
                  {char === ' ' ? '\u00A0' : char}
                </span>
